...
const NavButton = styled.button<{
  ...
  background-color: ${(props) =>
    props.$ordered || props.$preparation || props.$delivery || props.$completed
...
const OrdersManagement = () => {
  ...
  const [ordersQuantity, setOrdersQuantity] = useState<{
    ...
    delivery: number;
    completed: number;
  }>({ ordered: 0, preparation: 0, delivery: 0, completed: 0 });
    ...
    const getOrdersQuantity = (orders: Array<Order> | null) => {
      if (orders && orders.length > 0) {
        const quantity = {
          ...
          preparation: orders.filter((order) => order.status === 'preparation')
            .length,
      ...
      <PageContainer title="Orders Management" small>
        <Content>
          <div className="options">
            <SortOption>
              <SortButton onClick={() => setSortOption(!sortOption)}>
                Sort
              ...
              {sortOption && (
                <Dropdown>
                  <DropdownItem
                    onClick={() => dispatchSort({ type: 'recencyDESC' })}
                  >
                    By recency (Oldest to newest)
                  </DropdownItem>
                  <DropdownItem
                    onClick={() => dispatchSort({ type: 'recencyASC' })}
                  >
          ...
          <div className="header">
            <Nav>
              <NavButton
                ...
                onClick={() => dispatch({ type: 'ordered' })}
              >
                Ordered{' '}
                <OrdersQuantity>{ordersQuantity.ordered}</OrdersQuantity>
              </NavButton>
              <NavButton
                $preparation={state === 'preparation'}
                onClick={() => dispatch({ type: 'preparation' })}
              >
                Preparation{' '}
                <OrdersQuantity>{ordersQuantity.preparation}</OrdersQuantity>
              </NavButton>
              <NavButton
              ...
              >
                Delivery{' '}
                <OrdersQuantity>{ordersQuantity.delivery}</OrdersQuantity>
              </NavButton>
              <NavButton
              ...
              >
                Completed{' '}
                <OrdersQuantity>{ordersQuantity.completed}</OrdersQuantity>
              </NavButton>
            </Nav>
            <Header
              $ordered={state === 'ordered'}
          ...
          <div className="list-container">
            {orders?.filter((order) => order.status === state).length === 0 && (
              <Empty>{`No orders with ${state} status found`}</Empty>
            )}
            {orders
                ...
                <ListItem key={order.orderID}>
                  <div className="content">
                    ...
                    <div className="order-status">
                      <StatusButton
                        onClick={() =>
                          statusDropdown === null
                            ? setStatusDropdown(order.orderID)
                      ...
                      {statusDropdown === order.orderID && (
                        <StatusDropdown>
                          <StatusDropdownItem
                            onClick={() => {
                              changeStatus(order.orderID, 'ordered');
                              setStatusDropdown(null);
                            }}
                          >
                            ordered
                          ...
                          <StatusDropdownItem
                            onClick={() => {
                              changeStatus(order.orderID, 'preparation');
                              setStatusDropdown(null);
                            }}
                          >
                            preparation
                          ...
                          <StatusDropdownItem
                            onClick={() => {
                              changeStatus(order.orderID, 'delivery');
                              setStatusDropdown(null);
                            }}
                          >
                            delivery
                          ...
                          <StatusDropdownItem
                            onClick={() => {
                              changeStatus(order.orderID, 'completed');
                              setStatusDropdown(null);
                            }}
                          >
                            completed
                    ...
                    <button
                      ...
                      onClick={() =>
                        setDetails(
                          details === order.orderID ? null : order.orderID
                        )
                      }
                    >
                  ...
                  {details === order.orderID && (
                    <div className="details">
                      <div>
                        <h4>Customer</h4>
                        <p>{getFullname(order.customerID, users)}</p>
                      </div>
                      <div>
                        <h4>Phone number</h4>
                        <p>{getPhoneNumber(order.customerID, users)}</p>
                      </div>
                      <div>
