...
const StepContainer = styled.div<{ $active?: boolean }>`
  ...
  > div.step-details {
    border-radius: 10px;
    border: ${(props) =>
      props.$active ? 'none' : `1px solid ${props.theme.colors.neutral[200]}`};
    flex: 1;
...
const ShoppingCartItems = styled.div<{ $active?: boolean }>`
  border-bottom: ${(props) =>
    props.$active ? `1px solid ${props.theme.colors.neutral[200]}` : 'none'};
`;
...
const OrderSummary = () => {
  ...
  const [activeButtonPayment, setActiveButtonPayment] =
  ...
  const formikPromotion = useFormik({
    ...
    onSubmit: (values) => {
      if (promotions) {
        const promotionFind = promotions.find(
          (promo) => promo.type === values.promotion
        );
        if (promotionFind) {
          ...
          formikPromotion.setFieldError(
            'promotion',
            'The promotion code is incorrect'
          );
        }
      } else {
        formikPromotion.setFieldError(
    ...
    <Container>
      <PageContainer title="Order summary">
                      ...
                      <Button
                        ...
                        onClick={() => {
                          ...
                          setOrder({
                            ...order,
                            products: shoppingCartItems,
                            price: totalPrice(shoppingCartItems, promotionType),
                          });
                        }}
                      >
                        Next
                  ...
                  <Button
                    ...
                    onClick={() => {
                      ...
                      order.deliveryAddress
                        ? dispatch({ type: 'SET_STEP2', payload: 'false' })
                        : dispatch({ type: 'SET_STEP2', payload: 'disabled' });
                      order.paymentMethod
                        ? dispatch({ type: 'SET_STEP3', payload: 'false' })
                        : dispatch({ type: 'SET_STEP3', payload: 'disabled' });
                    }}
                  />
                </HeaderEdit>
                <ShoppingCartItems>
                ...
                <Form onSubmit={formik.handleSubmit}>
                  {userData?.addressesList?.slice(0, 3).map((address) => (
                    <InputRadio key={address.addressID}>
                      ...
                      />
                      <label htmlFor={`${address.addressID}`}>
                        {address.address}
                      </label>
                    </InputRadio>
                  ))}
            ...
            <div className="step-details">
              <SectionContainer>
                ...
                >
                  <img src={iconPin} alt="" />
                  <p>
                    {order?.deliveryAddress || 'No delivery address available'}
                  </p>
                </div>
              </SectionContainer>
            </div>
        ...
        {!state.step1 && state.step2 === 'false' && state.step3 === 'false' && (
          <PageFooter>
            <Button buttonType="textBlack" onClick={() => navigate('/')}>
              Cancel
            </Button>
            <Button buttonType="primary" onClick={() => createOrder(order)}>
              Order now
            </Button>
          </PageFooter>
        )}
